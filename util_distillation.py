...
def get_margin_logits(logits, y):
    # logits: (batch, num_classes), y: (batch,)
    batch_indices = torch.arange(logits.size(0), device=logits.device)
    correct_logits = logits[batch_indices, y].unsqueeze(1)  # (batch, 1)
    tmp = logits.clone()
    tmp[batch_indices, y] = float("-inf")
    max_other_logits = tmp.max(dim=1, keepdim=True)[0]  # (batch, 1)
    return torch.cat([correct_logits, max_other_logits], dim=1)  # (batch, 2)


def imitate_out_train(teacher_model, train_loader, test_loader, device, args):
    """Train model using distillation from teacher model"""
    ...
    for i in range(args.epochs):
        ...
        for itr, (x, y) in enumerate(pbar):
            ...
            if i < args.warmup_epochs:
                ...
                # Calculate distillation loss (either MSE or KL)
                if args.mse_distillation:
                    # MSE distillation with margin focus
                    ...
                    distill_loss = mse_criterion(student_logits * weight_matrix, teacher_logits * weight_matrix)

                    # Normalize MSE loss to be in similar scale as CE loss
                    avg_weight = weight_matrix.mean()
                    distill_loss = distill_loss / avg_weight
                else:
                    # KL divergence based distillation
                    distill_loss = kldiv_criterion(
                        F.log_softmax(student_logits / args.temperature, dim=1),
                        F.softmax(teacher_logits / args.temperature, dim=1),
                    ) * (args.temperature**2)

                if np.isnan(distill_loss.item()):
                    raise ValueError("distill_loss is nan")

                # Combine losses using alpha
                loss = args.alpha * distill_loss + (1.0 - args.alpha) * ce_loss
            ...
            if i < args.warmup_epochs:
                ...
                pbar.set_postfix_str(
                    f"loss: {loss.item():.2f}, ce_loss: {ce_loss.item():.2f}, distill_loss: {distill_loss.item():.2f}"
                )
...
def distill_acc_rule(args):
    if args.dataset == "cifar10":
        args.distill_acc = 0.6  # 0.85
    elif args.dataset == "cinic10":
        args.distill_acc = 0.4  # 0.8
    elif args.dataset == "cifar100":
        args.distill_acc = 0.4  # 0.7
    elif args.dataset == "mnist" or args.dataset == "fmnist":
        args.distill_acc = 0.8  # 0.9
    else:
