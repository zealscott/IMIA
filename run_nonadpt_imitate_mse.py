...
for shadow_id in range(args.n_imitate_shadows):
    if shadow_id % args.n_gpus == args.cuda:
        ...
        cmd = f"python imitate_nonadapt_separate_mse.py --dataset {args.dataset} --shadow_id {shadow_id} --data_dir {args.data_dir} --savedir {args.savedir} --seed {args.seed} --model_type {args.model_type} --n_imitate_shadows {args.n_imitate_shadows} --temperature {args.temperature}"
        os.system(cmd)
