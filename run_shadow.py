...
for shadow_id in range(0, args.n_shadows):
    if shadow_id % args.n_gpus == args.cuda:
        ...
        os.system(
            f"python shadow.py --dataset {args.dataset} --shadow_id {shadow_id} --data_dir {args.data_dir} --savedir {args.savedir} --seed {args.seed} --model_type {args.model_type} --n_shadows {args.n_shadows}"
        )
