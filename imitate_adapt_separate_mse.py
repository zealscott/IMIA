...
import torch
from util_distillation import distill_acc_rule, imitate_out_train, select_pivot_data, imitate_in_train
...
# parser.add_argument("--imitate_in_epochs", default=50, type=int)
...
parser.add_argument("--margin_mse_distillation", default=False, type=bool)
parser.add_argument("--distill_acc", default=0.5, type=float)
...
def train_and_save_distill(model_type):
    ...
    distill_acc_rule(args)

    #########################################################
    # prepare data
    #########################################################
    # Load shadow dataset for distillation
    ...
    # train out model
    #########################################################
    # distill shadow model using nonpivot data
    ...
    if test_acc < args.distill_acc:  # Use fixed threshold instead of args.distill_acc
...
def inference_all(savedir, data_type, model_name):
    ...
    np.save(os.path.join(savedir, f"{model_name}_losses_on_{data_type}.npy"), losses_n)
...
if __name__ == "__main__":
    ...
    args.margin_weight = 1.0
    # num_classes = 100 if args.dataset == "cifar100" else 10
    # args.margin_weight = np.log(num_classes)
    # args.margin_weight = np.sqrt(num_classes)
    ...
    model_name = f"mse_alpha{args.alpha:.1f}_temp{args.temperature:.1f}_weight{args.margin_weight:.1f}"
    ...
    #     train_and_save_distill(model_name)

    data_type = "shadow"
    if not os.path.exists(os.path.join(run_savedir, f"adapt_out_{model_name}_losses_on_{data_type}.npy")):
